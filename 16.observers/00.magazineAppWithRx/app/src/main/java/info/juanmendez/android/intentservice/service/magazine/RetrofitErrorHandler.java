...
/**
 * Converts the complex error structure into a single string you can get with
 * error.getLocalizedMessage() in Retrofit error handlers. Also deals with there being no network
 * available
 *
 * <p>Uses a few string IDs for user-visible error messages
 *
  ...
  public Throwable handleError(RetrofitError cause) {
    ...
    if (cause.getKind() == RetrofitError.Kind.NETWORK) {
      ...
      if (cause.getResponse() == null) {
        ...
        try {
          RetrofitErrorResponse errorResponse =
              (RetrofitErrorResponse) cause.getBodyAs(RetrofitErrorResponse.class);

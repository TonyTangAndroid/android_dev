...
public class MagazineListService extends IntentService {
  ...
  @Inject MagazineNotificationSubject notificationSubject;
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    try {
      ...
      for (Magazine m : magazines) {
        ...
        if (result != null) addedMagazines.add(m);

        dirty |= (result != null);
      }

      if (dirty) log.setState(Log.Integer.DIRTY);
      else log.setState(Log.Integer.CLEAN);

      magazinesNotification.setResultCode(Activity.RESULT_OK);

      if (rec != null) rec.send(Activity.RESULT_OK, new Bundle());

    } catch (Exception e) {
      e.printStackTrace();

      if (rec != null) rec.send(Activity.RESULT_CANCELED, new Bundle());
        ...
        new NotificationCompat.Builder(this)
            ...
            .setContentIntent(
                PendingIntent.getActivity(
                    ...
                    0,
                    new Intent(this, ListMagazinesActivity.class),
